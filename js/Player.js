define(["Keyboard", "Vector2D", "Sprite", "Communicator", "Gamepad"], function(Keyboard, Vector2D, Sprite, Communicator, Gamepad) {"use strict";
	var Player = function Player() {
		...
		Communicator.instance.register(this, "push", function(data) {
			console.log("got a push thingie");
			this.push(data);
		});
	};
	...
	Player.prototype.push = function(vec) {
		new TWEEN.Tween({
			x : Player.instance.position.x,
			y : Player.instance.position.y
		}).to({
			x : Player.instance.position.x + vec.x,
			y : Player.instance.position.y + vec.y
